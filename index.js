...
function getSymbolKey(symbol) {
  const sym = Symbol.keyFor(symbol);
  return sym;
...
// Об'єкт "myObject" представляє значення   from: 1, to: 6, які можна перебрати
let myObject = {
  from: 1,
  to: 6,
...
console.log(useSymbolIterator(myObject)); //Виведе [ 1, 2, 3, 4, 5, 6 ]
...
function useSymbolIterator2(obj) {
  let result = [];
  for (let value of obj) {
    result.push(value);
  }
...
console.log(useSymbolIterator2(myObject2)); //Виведе [ 'apple', 'banana', 'orange', 'grape', 'mango' ]
